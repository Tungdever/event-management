...
const RoleManagement = ({ token, permissions, fetchPermissions }) => {
  ...
  const createRole = async (roleData) => {
    try {
      setError(null);
      const response = await fetch('https://utevent-3e31c1e0e5ff.herokuapp.com/api/roles', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const deleteRole = async (roleName) => {
    try {
      setError(null);
      const response = await fetch(`https://utevent-3e31c1e0e5ff.herokuapp.com/api/roles/delete/${roleName}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const assignPermissionsToRole = async (roleId, permissionNames) => {
    try {
      setError(null);
      const response = await fetch(`https://utevent-3e31c1e0e5ff.herokuapp.com/api/roles/${roleId}/permissions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
