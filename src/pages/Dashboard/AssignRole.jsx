...
const AdminRoleAssignment = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        const response = await fetch('https://utevent-3e31c1e0e5ff.herokuapp.com/api/roles/roles-assign', {
          headers: {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setError(null);
        if (!token) throw new Error(t('adminRoleAssignment.errorNoToken'));
        const response = await fetch(`https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/get-all-event-by-org/${user.email}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  const searchUsers = async (searchQuery) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://utevent-3e31c1e0e5ff.herokuapp.com/chat/search?query=${encodeURIComponent(searchQuery)}&currentUserId=${user.userId}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
  ...
  const deleteAssignedRole = async (userId, roleName, eventId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (!token) throw new Error(t('adminRoleAssignment.errorNoToken'));
        const response = await fetch(
          `https://utevent-3e31c1e0e5ff.herokuapp.com/api/role-assignment/delete?userId=${userId}&roleId=${roleId}&eventId=${eventId}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
  ...
  const assignRole = async () => {
    ...
    try {
      setError(null);
      if (!token) throw new Error(t('adminRoleAssignment.errorNoToken'));
      const response = await fetch('https://utevent-3e31c1e0e5ff.herokuapp.com/api/role-assignment/assign-role', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const fetchTeamMembers = async (eventId) => {
    ...
    try {
      ...
      setError(null);
      if (!token) throw new Error(t('adminRoleAssignment.errorNoToken'));
      const response = await fetch(`https://utevent-3e31c1e0e5ff.herokuapp.com/api/role-assignment/${eventId}/my-team-events`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
