...
const EventsPage = () => {
  ...
  const fetchEventData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/get-all-event-by-org/${user.email}`,
        {
          headers: {
  ...
  const deleteEvent = async (eventId) => {
    ...
    try {
      const response = await fetch(
        `https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/delete/${eventId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const publishEvent = async (eventId) => {
    setLoading(true);
    try {
      const response = await fetch(`https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/publish/${eventId}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
