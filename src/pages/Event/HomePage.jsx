  ...
  const fetchEventsByCity = async (citySlug) => {
    try {
      const response = await fetch(
        `https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/search/by-city/${citySlug}`);
      if (!response.ok) {
...
const TopDestinations = () => {
  ...
  useEffect(() => {
    const fetchTopCities = async () => {
      try {
        const response = await fetch(
          "https://utevent-3e31c1e0e5ff.herokuapp.com/api/events/search/top-cities-popular"
        );
