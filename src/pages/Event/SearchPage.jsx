...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchTopCities = async () => {
      try {
        const response = await fetch("https://event-management-server-asi9.onrender.com/api/events/search/top-cities-popular");
    ...
    const fetchEvents = async () => {
      ...
      try {
        const response = await fetch(
          `https://event-management-server-asi9.onrender.com/api/events/search/multiple-filters?eventCategory=${selectedCategories}&eventLocation=${selectedEventLocation}&eventStart=${selectedEventStart}&ticketType=${selectedTicketType}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
